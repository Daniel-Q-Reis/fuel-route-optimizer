...
class ORSClientTest(TestCase):
    ...
    def test_successful_geocoding(self, mock_get):
        ...
        lat, lon = self.client.geocode("123 Highway Rd, Springfield, IL")
    ...
    def test_geocoding_no_results(self, mock_get):
        ...
        with self.assertRaises(GeocodingError) as context:
            self.client.geocode("Invalid Address XYZ123")
    ...
    def test_geocoding_timeout(self, mock_get):
        ...
        with self.assertRaises(GeocodingError) as context:
            self.client.geocode("123 Test St")
    ...
    def test_session_has_retry_strategy(self):
        ...
        self.assertIn("https://", self.client.session.adapters)
        self.assertIn("http://", self.client.session.adapters)

        # Check retry configuration
        https_adapter = self.client.session.adapters["https://"]
        self.assertIsNotNone(https_adapter.max_retries)
        self.assertEqual(https_adapter.max_retries.total, 3)
        self.assertEqual(https_adapter.max_retries.status_forcelist, [429, 500, 502, 503])
